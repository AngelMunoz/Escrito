...
routing.registerRoute(
  ({ event, request, url }) => ["/"].includes(url.pathname),
  new strategies.StaleWhileRevalidate({ cacheName: "html" })
);
routing.registerRoute(
  ({ event, request, url }) => url.pathname.includes("app.webmanifest"),
  new strategies.StaleWhileRevalidate({ cacheName: "manifest" })
);
